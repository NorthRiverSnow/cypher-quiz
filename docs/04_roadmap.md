...
**GitHub Pages に web だけ置く。** クイズは端末の中で完結する（model は純関数、保存は
localStorage）ので、静的ホスティングで全機能が動く。

**`packages/api` は置かない。** api は**利用者が入力した接続先へ繋ぎに行くプロキシ**で、
[歯止め](./03_api.md#歯止め)が「既定はローカル宛のみ」を前提にしている。公開すると、
誰でも任意のホストへ Bolt で繋げる踏み台になる。**クエリを実行したい人は手元で `vp run dev`**。
...
| | |
|---|---|
| 返るのは 404 のまま | 画面は出るが、ステータスは 404。クローラは「無い」と判断する |
| 本物の 404 が消える | このアプリは元々 404 ページを持たない（知らない URL はスタートへ送る）ので、これだけなら困らない |
| **アセットを取り違える** | 古い `index.html` がブラウザに残っていると、消えたハッシュ付きファイルの要求に HTML が返り、白い画面になる |

`HashRouter` なら**サーバは `index.html` だけを返せばよく**、この 3 つが全て消える。
代償は URL に `#` が入ることだけで、配る URL の見た目としては許せる。

...
**太さは variant で決まる。上書きするのは、意味の強調が要るときだけ。**
`Text` の `weight` がその口で、今使っているのは**全問正解の章の数**（スタート画面と結果画面）だけ。
色（`tone`）と違い、太さは段階表の一部なので、**新しい用途を増やすときはこの表に書き足す。**
...
| `--weight-semibold` | 600 | mono | 肢の構文、正順の設問、コードの語句、全問正解の章の数 |
...
| カードの中のカード（章の一覧） | 同上 | 同上 + 上辺を切る見出し | 4px |
...
| 章の行の、全問正解の数 | なし（字は `--keep` の semibold） | なし | なし |

**章の行の数は、全問正解のときだけ `--keep` の semibold。** 結果画面の章別成績と同じ出し方で、
一覧の中でその行だけが浮く。他の行は `--ink-soft` の regular のままにする——
全ての行に色を付けると、どれが済んだ章か分からなくなる。

**章の一覧はくぼませない。** スタート画面では説明の文章と並ぶので、沈めると
読み飛ばす部分に見える。同じ面のまま罫線と影で囲い、選ぶ場所だと分かるようにする。

**カードの見出しは上辺の罫線を切って重ねる。** 字は `annotation` の `--ink-soft`、
左は内側の余白（`--space-lg`）から始め、切れ目は文字の左右に `--space-xs`。
**上の余白は見出しの高さの半分だけ減らす**——見出しは罫線の上に半分はみ出して乗るので、
そのままだと罫線から中身までが左右より広くなる。
`fieldset` と `legend` で組むので、ラベルの背面に面を敷かずに済む——
外側がどの面でも合う。

...
| **ログに出さない** | リクエストボディを出す口を作らない。URI と Cypher は[資格情報を取り除いてから](#redactts-が取り除くもの)出す |
| **エラーをサニタイズ** | ドライバのエラーは URI に資格情報を含みうる。クライアントへ返す前に取り除く |
| **メモリのみ** | サーバ側 `Map`。**サーバ再起動で全消滅**。保管庫は `password` を持たない（ドライバは再接続のために内部に持つ） |
...
**パスワードを暗号化しない。** 保存しないので、暗号化する対象が無い。守るのは経路のほう。

| 経路 | 今 |
|---|---|
| ブラウザ → api | dev は http（同一の機械）。**本番配布するなら HTTPS が要る**（クッキーの `Secure` はその前提） |
| api → Neo4j | **ローカル以外は必ず暗号化される**（下） |
| api のメモリ | 保管庫は `password` を持たない。ドライバは再接続のために持つ。プロセス終了で消える |
| ログ・応答 | [取り除いてから出す](#redactts-が取り除くもの) |

### ローカル以外は暗号化スキームに繋ぎ変える

`neo4j-driver` の暗号化は **URI のスキームで決まる**（設定では変えられない）。
`api/src/neo4j/uri.ts` が繋ぐ前に決める。
...
| `http://…` / パースできない文字列 | `invalid-request`（422）。**ドライバを作らない** |
| `bolt://user:pass@host` | `invalid-request`（422）。資格情報は body で受け取る |

**拒否ではなく繋ぎ変える。** 暗号化に対応した相手（Aura など）ならそのまま繋がり、
対応していなければドライバが失敗する——**どちらにしても平文では出ない。**

**返す `ConnectionStatus.uri` は繋ぎ変えたあとのもの。** 入力を返すと、画面が
「`bolt://` で繋がっている」と嘘をつく。
...
`req.start` が出すのは `method` と `path` だけ。**クエリ文字列は出ない**（Hono の
`c.req.path` はパスだけを返す。実測）。**body を渡す引数を作らない**
——引数があると、いつか誰かが渡す。
...
- **`redact.ts` を通してから切る。** 利用者はクエリを編集して実行できるので、
  `LOAD CSV FROM 'https://u:p@…'` も `CREATE USER … SET PASSWORD '…'` も書ける。
  拒否したクエリも残す以上、**書かれた資格情報は出る前に伏せる**

### `redact.ts` が取り除くもの

| 形 | 例 | 出るもの |
|---|---|---|
| URI に埋まった資格情報 | `bolt://neo4j:pw@db:7687` | `bolt://db:7687` |
| Cypher のパスワード | `SET PASSWORD 'pw'` | `SET PASSWORD '***'` |
| パスワードの変更 | `SET PASSWORD FROM 'a' TO 'b'` | `SET PASSWORD FROM '***' TO '***'` |

**切る前に取り除く。** 順番が逆だと、切れ目をまたいだ URI が `@` を失って一致しなくなり、
**パスワードだけが残る。**

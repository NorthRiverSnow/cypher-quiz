...
| 経路 | 今 |
|---|---|
| ブラウザ → api | dev は http。**本番配布するなら HTTPS が要る**（クッキーの `Secure` はその前提） |
| api → Neo4j | **URI のスキーム次第。** `neo4j+s` / `bolt+s`（Aura）は TLS、`bolt://`（ローカル）は平文 |
| api のメモリ | 保管庫は持たない。ドライバは再接続のために持つ。プロセス終了で消える |
| ログ・応答 | [取り除いてから出す](#redactts-が取り除くもの) |

**`ConnectRequestSchema` はスキームを見ていない。** 利用者が自前の Neo4j に `bolt://` で
繋ぐと、パスワードが平文でネットワークに出る。塞ぐなら
[C-6 の歯止め 2](#歯止め)（ローカル宛の判定）を同じ場所で使い回せる——[判断待ち](./04_roadmap.md#前提として置いた判断)。

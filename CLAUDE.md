...
**名前から読み取れないものだけ** `/** */` で書く。呼ぶ側が知る必要のある振る舞い
（短絡する、例外を throw する、副作用がある）が対象。

```
✗ /** id で製品を削除する */  deleteProductById
○ /** 失敗なら fn を呼ばず、最初の理由をそのまま返す */  flatMap
```

`why:` はその中に続けて書いてよい。**@param と @returns は書かない**——型で分かる。
